...
def trace_to_8k(trace_path, trace_type='MSRC'):
    ...
    output_dir = 'dataset/8k_lba_traces'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # Save the processed trace
    save_file = os.path.join(output_dir, str(trace_path.split('/')[-1].split('.')[0]) + '.csv')
...
def seperate_8k_by_64M(trace_path):
    ...
    dir_64M =  os.path.join('dataset/64M_sep/',str(trace_path.split('/')[-1].split('.')[0]))
...
def session_generate(trace_path):
    ...
    pickle.dump(df_list_reshape,open(os.path.join('dataset/sessions/',trace_path.split('/')[-1]), 'wb'))
    id_64m_list =pd.DataFrame(id_64m_list)
    id_64m_list.to_csv(os.path.join('dataset/sessions/','64m_id'+trace_path.split('/')[-1]),index=None,header=None)
    return 0 
...
def delta_session_generate(trace_path):
    ...
    df_list_reshape = [[item[i] for item in df_list] for i in range(4) ]
    print('generate finish')
    pickle.dump(df_list_reshape,open(os.path.join('dataset/delta_sessions/',trace_path.split('/')[-1]), 'wb'))
    id_64m_list =pd.DataFrame(id_64m_list)
    id_64m_list.to_csv(os.path.join('dataset/delta_sessions/','64m_id'+trace_path.split('/')[-1]),index=None,header=None)
    return 0 
...
def full_delta_session_generate(trace_path):
    ...
    df_list_reshape = [[item[i] for item in df_list] for i in range(4) ]
    print('generate finish')
    pickle.dump(df_list_reshape,open(os.path.join('dataset/full_delta_sessions/',trace_path.split('/')[-1]), 'wb'))
    id_64m_list =pd.DataFrame(id_64m_list)
    id_64m_list.to_csv(os.path.join('dataset/full_delta_sessions/','64m_id'+trace_path.split('/')[-1]),index=None,header=None)
    return 0 
if __name__ == '__main__':
    ...
    if opt.type =='full' or opt.type =='8k':
        ...
        for type_dir in os.listdir(dataset_dir):
                print(type_dir)
                if type_dir =='VDI512' or type_dir == 'MSRC':
                    ...
                    for trace_name in os.listdir(dataset_type_dir):
                        ...
                        if os.path.isfile(trace_path):
                            print('Processing original trace to 8KB LBA trace, trace name:', trace_path)
                            trace_to_8k(trace_path=trace_path, trace_type=type_dir)

    if opt.type =='full' or opt.type =='sep':
        dataset_dir = 'dataset/8k_lba_traces/'
